...
export default function BillingPage() {
  ...
  const handleConfirmPayment = async () => {
    if (selectedBill && selectedMethod) {
      await dispatch(markBillPaid({ id: selectedBill.id, paymentMethod: selectedMethod }));
      toast.success('Payment Successful', `Payment received via ${selectedMethod}`);
      setPaymentStep(2);
      // Auto-send review link after payment
      setTimeout(async () => {
        try {
          const token = await simulateSendReviewNotification(dispatch, selectedBill);
          ...
          toast.error('Review link failed', err instanceof Error ? err.message : 'Could not generate review link');
        }
      }, 1500);
    }
  };
