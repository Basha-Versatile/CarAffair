...
export default function VehiclesPage() {
  const { vehicles, customers, isLoading } = useAppSelector((state) => state.customers);
              ...
              <Card hover>
                ...
                <div className="space-y-2.5 text-sm">
                  ...
                  <div className="flex items-center justify-between p-2.5 rounded-lg bg-[var(--bg-tertiary)]">
                    ...
                    <span className="text-[var(--text-primary)] font-medium">{(vehicle.mileage ?? 0).toLocaleString()} km</span>
                  </div>
                  <div className="flex items-center justify-between p-2.5 rounded-lg bg-[var(--bg-tertiary)]">
              ...
              <tbody className="divide-y divide-[var(--border-color)]">
                {paginatedVehicles.map((vehicle, idx) => (
                  <motion.tr key={vehicle.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.03 }} className="hover:bg-[var(--bg-glass-hover)] transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-[var(--text-primary)]">{(vehicle.mileage ?? 0).toLocaleString()} km</td>
