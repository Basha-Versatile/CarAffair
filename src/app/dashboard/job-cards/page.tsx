...
import { generateId, formatDate, formatCurrency } from '@/utils/format';
...
export default function JobCardsPage() {
  ...
  const { jobCards, statusFilter, searchQuery, currentPage, pageSize } = useAppSelector((state) => state.jobCards);
  ...
  const handleSubmit = (data: JobCardFormData) => {
    ...
    if (editingJob) {
      ...
      dispatch(addJobCard(newJob));
      toast.success('Job Card Created', `New job for ${data.vehicleName} created`);
      simulateSendJobCreatedNotification(dispatch, newJob);
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-[var(--text-tertiary)]">{v.color} &middot; {v.mileage.toLocaleString()} km</p>
                          </div>
                        </div>
